...
public class StaffService {
    ...
    public static void readFileStaff() throws FileNotFoundException, ParseException {
    ...
    public static void writeStaffIntoFile(Staff staff) throws IOException {
        File file = new File(fileStaffs);
        FileWriter fileWriter = new FileWriter(file,true);
        fileWriter.write(String.format("%d#%s#%d#%s#%s#%s#%s\n",
                staff.getPk(),staff.getName(),staff.getSex().getInt(),
                staff.getHomeTown(),
                Utils.getSimpleDateFormat().format(staff.getDob()),Utils.getSimpleDateFormat().format(staff.getJoinDate()),
    ...
    public static void writeListStaffIntoFile() throws IOException {
        File file = new File(fileStaffs);
        FileWriter fileWriter = new FileWriter(file);
        fileWriter.close();
        StaffService.listStaff.forEach(st ->{
            try {
                StaffService.writeStaffIntoFile(st);
            } catch (IOException e) {
                System.out.println(e.getMessage());
            }
        });
    ...
    public static void updateStaffInfoByID(int pk, String name, String homeTown, Gender gender, Date dob, Date joinDate, Department department) throws IOException {
        if(StaffService.findStaffByID(pk) != -1){
            ...
            StaffService.writeListStaffIntoFile();
        }
    }
